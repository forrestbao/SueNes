...
from antirouge.serial_preprocess import glob_sorted
...
class SequenceWrapper(keras.utils.Sequence):
    def __init__(self, files, file_batch_size=3, batch_size=1000, neg_size=5):
        self.files = files
...
class Batch():
    def __init__(self, files, file_batch_size=3, batch_size=1000,
                 neg_size=5):
    ...
    def __iter__(self):
        ...
        for idx in range(file_batch_num):
            ...
            if isinstance(articles[0], str):
                ...
                maxlen_article = MAX_ARTICLE_LENGTH
                maxlen_summary = MAX_SUMMARY_LENGTH
            else:
                maxlen_article = ARTICLE_MAX_SENT
                maxlen_summary = SUMMARY_MAX_SENT
...
def get_data_generators(folder):
    ...
    neg_size = 1
    training_seq = SequenceWrapper(training_files, 3, 1000, neg_size)
    validation_seq = SequenceWrapper(validating_files, 3, 1000, neg_size)
    testing_seq = SequenceWrapper(testing_files, 3, 1000, neg_size)
...
def train_model(model, data_folder):
    ...
    mc = keras.callbacks.ModelCheckpoint('best_model.ckpt',
                                         ...
                                         save_best_only=True)
    model.fit_generator(training_seq, epochs=50, shuffle=False,
...
def exp_word():
    folder = os.path.join(SERIAL_DIR, 'story')
    model = get_word_model()
    train_model(model, folder)

def exp_USE():
    folder = os.path.join(SERIAL_DIR, 'USE')
    model = get_sent_model(512)
    train_model(model, folder)
def exp_USE_Large():
    folder = os.path.join(SERIAL_DIR, 'USE-Large')
    model = get_sent_model(512)
    train_model(model, folder)
def exp_InferSent():
    folder = os.path.join(SERIAL_DIR, 'InferSent')
...
InferSent."""
    article_input = keras.layers.Input(shape=(ARTICLE_MAX_SENT, embedding_size),
                                       dtype='float32')
    summary_input = keras.layers.Input(shape=(SUMMARY_MAX_SENT, embedding_size),
                                       dtype='float32')
...
def get_word_model():
    ...
    article_input = keras.layers.Input(shape=(MAX_ARTICLE_LENGTH,),
                                       dtype='int32')
    summary_input = keras.layers.Input(shape=(MAX_SUMMARY_LENGTH,),
                                       dtype='int32')
    x = keras.layers.concatenate([article_input, summary_input], axis=1)
    x = glove_layer(x)
    x = keras.layers.Flatten()(x)
