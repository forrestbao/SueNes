...
flags.DEFINE_integer("test_batch_size", 1, "Total batch size for eval.")
...
class MutationProcessor(DataProcessor):
    def __init__(self):
        self.f_train = 'mutate-add-train2.tsv'
        self.f_test = 'mutate-add-test2.tsv'
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            # label = float(line[4])
            text_a, text_b, label = None, None, None
            guid = "%s-%s" % (set_type, i)
            #try:
            #assert len(line[0].split()) > 400
...
class MutationDelProcessor(MutationProcessor):
    def __init__(self):
        self.f_train = 'mutate-delete-train2.tsv'
        self.f_test = 'mutate-delete-test2.tsv'
        self.f_predict = 'TAC2010_test.tsv'


class MutationRepProcessor(MutationProcessor):
    def __init__(self):
        self.f_train = 'mutate-replace-train2.tsv'
        self.f_test = 'mutate-replace-test2.tsv'
        self.f_predict = 'TAC2010_test.tsv'

class MutationMixProcessor(MutationProcessor):
    def __init__(self):
        self.f_train = 'mix.tsv'
        self.f_test = 'mutate-replace-test2.tsv'
        self.f_predict = 'TAC2010_test.tsv'
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, logits):
                ...
                return {'mean_pred': tf.metrics.mean(logits),
                        'mean_labels': tf.metrics.mean(label_ids),
                        'pearson': pearson,
                        'spearman': spearman,
                        'MSE': mse}
...
def main(_):
    ...
    processors = {
        "mutation-add": MutationProcessor,
        "mutation-delete": MutationDelProcessor,
        "mutation-replace": MutationRepProcessor,
        "mutation-mix": MutationMixProcessor
    }
    ...
    if FLAGS.do_test:
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                ...
                writer.write(str(probabilities) + "\n")
                num_written_lines += 1
